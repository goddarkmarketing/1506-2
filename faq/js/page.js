(() => {
  ...
  qsa("main button[aria-expanded], section button[aria-expanded]").forEach((btn) => {
    ...
    // Category groups on FAQ stay open (left/right columns)
    const isCat =
      btn.hasAttribute("data-dg-faq-cat") ||
      /Before booking a tour|On the tour day/i.test(btn.textContent || "");
    if (isCat) {
      const panel =
        btn.nextElementSibling ||
        btn.closest("h2")?.nextElementSibling ||
        btn.parentElement?.nextElementSibling;
      btn.setAttribute("aria-expanded", "true");
      btn.setAttribute("data-dg-faq-cat", "1");
      if (panel) {
